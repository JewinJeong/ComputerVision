...
def laws_texture(gray_image):
    ...
    smooth_kernel = (1/25)*np.ones((5,5)) #mak
...
for epoch in range(n_epoch):
    ...
    if (epoch+1) % 1 == 0:
        ...
        print('[%d, %3d]\tloss: %.4f\tAccuracy : %.4f\t\tval-loss: %.4f\tval-Accuaracy : %.4f' %(epoch+1, n_epoch, train_loss, train_acc, test_loss, test_acc))

plt.plot(range(len(train_losses)-1), train_losses, label='train loss')
plt.plot(range(len(test_losses)-1), test_losses, label='test loss')
plt.legend()
plt.plot(range(len(train_accs)-1), train_accs, label='train acc')
plt.plot(range(len(test_accs)-1), test_accs, label='test acc')
plt.legend()
plt.show()
